...
namespace ViewROI
{
	...
	public class HWndCtrl
	{
        ...
        public const int MODE_VIEW_ZOOMANDMOVE = 14;
        ...
        {
            double scale = 1;
            if (stateView == MODE_VIEW_ZOOM || stateView == MODE_VIEW_ZOOMANDMOVE)
            {
                if (e.Delta > 0)
                {
                    scale = 0.9;
		...
		private void mouseMoved(object sender, HalconDotNet.HMouseEventArgs e)
		{
			...
			else if (stateView == MODE_VIEW_MOVE || stateView == MODE_VIEW_ZOOMANDMOVE)
			{
				motionX = ((e.X - startX));
				motionY = ((e.Y - startY));
